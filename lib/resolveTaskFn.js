...
const interruptExecutionOnError = (ctx, execaChildProcess) => {
  ...
  ctx.events.once('lint-staged:error', errorListener)

  return () => {
    ctx.events.off('lint-staged:error', errorListener)
  }
}
...
const makeErr = (command, result, ctx) => {
  ...
  // Can't use just "error" because of special behavior
  // https://nodejs.org/api/events.html#error-events
  ctx.events.emit('lint-staged:error')
...
export const resolveTaskFn = ({
  ...
  return async (ctx = getInitialState()) => {
    ...
    const removeErrorListener = interruptExecutionOnError(ctx, execaChildProcess)
